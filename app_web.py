...
# ⭐️ 변경 유지: 동의어 구분을 위해 뉘앙스/특징을 필수 기재하도록 AI 명령어 강화
AI_PROMPT_RULES = """
[초강력 중요 규칙]
1. 번호나 리스트 표시 절대 금지. 줄바꿈 없이 한 단어당 한 줄로만 작성.
2. 영단어에 절대 ** 기호 금지.
3. 발음 기호: 국제음성기호(IPA) 표준을 따르고 반드시 대괄호 양옆에 공백을 한 칸씩 넣을 것! (예: [ klɑːs ])
4. 다품사 강제 & 뉘앙스 필수 요약(⭐️): 단어의 모든 주요 품사(명사, 동사, 형용사 등)를 찾되, 뜻은 품사별 1~2개만 간결하게 요약.
   - 단, 동의어(say, tell, speak 등) 구분을 위해 괄호 안에 (특징/뉘앙스)를 반드시 적어주세요!
   - (예: 명사 : 아래층 / 동사 : 말하다 (언어, 연설) / 동사 : 말하다 (특정 내용))
...
if menu == "🤖 AI 단어 생성":
    ...
    if st.button("🚀 단어 생성 시작"):
        ...
        with st.spinner("AI가 단어의 뉘앙스까지 구별하여 스캔 중입니다..."):
            try:
