...
if menu == "🤖 AI 단어 생성":
    ...
    if st.button("🚀 단어 생성 시작"):
        ...
        4. 다의어 및 다품사 '핵심 요약' (가장 중요⭐️): 단어가 가진 주요 품사는 모두 적되, 억지로 뜻을 지어내지 말고 각 품사별로 **가장 중요하고 자주 쓰이는 핵심 뜻 1~2개**만 간결하게 정확히 적으세요!
           - 같은 품사 내의 뜻은 쉼표(,)로 구분
           - 품사가 바뀌면 슬래시(/)로 구분
           - (예: 명사 : 학급, 수업 / 동사 : 분류하다)
        [형식]: 영단어;[발음기호];품사별 핵심 뜻;실전 예문 (예문은 단어당 1개만)
        """
        with st.spinner("AI가 핵심 뜻을 가진 단어를 생성 중입니다..."):
            try:
...
# ----------------- ✨ 수동 일괄 추가 -----------------
    ...
    if st.button("✅ 분석 및 추가"):
        if words_input:
            ...
            """
            with st.spinner("AI가 입력하신 단어의 핵심 품사를 스캔 중입니다..."):
                try:
                    response = get_ai_response(prompt)
                    lines = response.text.strip().split('\n')
                    new_rows = []
