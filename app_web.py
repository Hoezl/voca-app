...
if menu == "🤖 AI 단어 생성":
    ...
    if st.button("🚀 단어 생성 시작"):
        ...
        4. 다의어 및 다품사 강제 (가장 중요⭐️): 하나의 뜻만 적는 것을 엄격히 금지합니다. 단어가 가진 명사, 동사, 형용사 등 가능한 모든 품사와 다양한 뜻을 백과사전처럼 최대한 상세히 기재하세요!
           - 같은 품사 내의 뜻은 쉼표(,)로 구분
           - 품사가 바뀌면 슬래시(/)로 구분
           - (예: 명사 : 학급, 수업, 등급 / 동사 : 분류하다 / 형용사 : 일류의)
        [형식]: 영단어;[발음기호];품사별 상세 뜻;실전 예문
        """
        with st.spinner("AI가 상세한 사전식 뜻풀이를 생성 중입니다..."):
            try:
    ...
    if st.button("✅ 분석 및 추가"):
        if words_input:
            ...
            4. 다의어 및 다품사 강제 (가장 중요⭐️): 하나의 뜻만 적는 것을 엄격히 금지합니다. 단어가 가진 명사, 동사, 형용사 등 가능한 모든 품사와 다양한 뜻을 백과사전처럼 최대한 상세히 기재하세요!
               - 같은 품사 내의 뜻은 쉼표(,)로 구분
               - 품사가 바뀌면 슬래시(/)로 구분
               - (예: 명사 : 아래층 / 부사 : 아래층으로, 아래층에 / 형용사 : 아래층의)
            [형식]: 영단어;[발음기호];품사별 상세 뜻;실전 예문
            """
